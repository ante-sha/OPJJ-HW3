 ...
 * Objekte čuva kao elemente u dvostruko povezanoj listi. Dupliciranje elemenata 
...
public class LinkedListIndexedCollection implements List {
	...
	/** Konstruktor za stvaranje kolekcije sa svim elementima kolekcije other*/
	...
	/**
	 * {@inheritDoc}
	 * <br>Složenost metode je O(n) gdje je n broj elemenata.
	 */
	...
	public Object[] toArray() {
		...
		int i = 0;
		for(ListNode tmp = first;tmp!=null;tmp = tmp.right) {
	...
	/**
	 * {@inheritDoc}
	 * Element se dodaje na zadnju poziciju u složenosti O(1).
	 * Metoda ne dozvoljava unos {@code value} koji je null.
	 * 
	...
	public void add(Object value) {
		...
		ListNode novi = new ListNode(value,last,null);
		if(last == null) {
			first = novi;
		} else {
			last.right = novi;
	...
	/**
	 * Metoda koja vraća element kolekcije na index-tom mjestu.
	 * <br>Složenost metode je O(n) gdje je n broj elemenata u kolekciji.
	...
	public Object get(int index) {
		...
		ListNode cvor = getListNodeSaPozicije(index);
		return cvor.value;
	}
	
	/**
	 * Pomočna metoda za dohvat index-tog čvora u listi.
	 * <br>Složenost operacije je O(n) gdje je n broj elemenata.
	 * @param index index čvora za dohvat
	 * @return čvor s index-tog mjesta
	 */
	private ListNode getListNodeSaPozicije(int index) {
		provjeraRaspona(index,0,size-1);
		...
		ListNode tmp = početak ? first : last;
	...
	/**
	 * Metoda koja ubacuje element na poziciju {@code position}
	 * <br>Složenost metode je O(n) gdje je n broj elemenata u kolekciji. 
	...
	public void insert(Object value, int position) {
		...
		ListNode lijeviCvor = getListNodeSaPozicije(position-1);
		
		ListNode noviCvor = new ListNode(value,lijeviCvor,lijeviCvor.right.right);
	...
	/**
	 * Metoda koja vraća index elementa istog elementu value po uvjetima equals metode.
	 * <br>Složenost metode je O(n) gdje je n broj elemenata u kolekciji.
	...
	public int indexOf(Object value) {
		...
		int i = 0;
		for(ListNode tmp = first;tmp!=null;tmp = tmp.right,i++) {
	...
	/**
	 * Metoda koja uklanja element s index-tog mjesta.
	 * <br>Složenost ove metode je O(n) gdje je n broj elemenata u kolekciji.
	...
	public void remove(int index) {
		...
		ListNode lijeviCvor = getListNodeSaPozicije(index-1);
	...
	public boolean remove(Object value) {
		...
		for(ListNode tmp = first;tmp.right!=null;tmp=tmp.right) {
	...
	public ElementsGetter createElementsGetter() {
		return new LinkedElementsGetter(this,modificationCount);
	}
	...
	private void provjeraRaspona(int position,int pocetak, int kraj) {
		if(position < pocetak || position > kraj)
			throw new IndexOutOfBoundsException(
					String.format("%d nije u rasponu [0,%d]",position,pocetak,kraj)
					);
	}

	
	/**
	 * Pomočni razred za ostvarenje LinkedListIndexedCollection. ListNode je jedan atom linked liste.
	 * {@code null} reference označavaju kraj liste na toj strani.
	 * @author Ante Miličević
	 *
	 */
	private static class ListNode {
		 ...
		 */
		public ListNode(Object value,ListNode left,ListNode right) {
			this.value = value;
			this.left=left;
			this.right=right;
		}
	...
	/**
	 * Privatni razred LinkedListIndexedCollection koji omogućava iteriranje elementima kolekcije.
	...
	private static class LinkedElementsGetter implements ElementsGetter{
		...
		private LinkedListIndexedCollection kolekcija;
		 ...
		 */
		public LinkedElementsGetter(LinkedListIndexedCollection kolekcija,long modificationCount) {
			this.kolekcija = kolekcija;
			iduciCvor = kolekcija.first;
		...
		public Object getNextElement() {
			...
			Object rezultat = iduciCvor.value;
