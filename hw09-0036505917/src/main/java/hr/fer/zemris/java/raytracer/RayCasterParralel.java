 ...
 *
 */
public class RayCasterParralel {
	/**
	...
	private static IRayTracerProducer getIRayTracerProducer() {
		return new IRayTracerProducer() {
			...
			public void produce(Point3D eye, Point3D view, Point3D viewUp, double horizontal, double vertical,
				...
				Point3D G = view.sub(eye).modifyNormalize();

				Point3D yAxis = viewUp.sub(view).normalize();
				Point3D xAxis = G.vectorProduct(yAxis);
	...
	private static class RayTracerAction extends RecursiveAction {
		...
		private static void determineColorFor(Scene scene, RayIntersection closest, short[] rgb, Ray ray) {
			for (LightSource light : scene.getLights()) {
				...
				if (lightClosest == null || lightClosest.getPoint().sub(closest.getPoint()).norm() > 1e-6) {
					continue;
				}

				Point3D l = source.sub(closest.getPoint()).modifyNormalize();
				Point3D normal = closest.getNormal();
				Point3D reflected = normal.scalarMultiply(2 * l.scalarProduct(normal)).sub(l).normalize();
